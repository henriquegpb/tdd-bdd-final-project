...
from service.common import status  # Corrected import
...
# Define the check_content_type function
...
def update_products(product_id):
    ...
    app.logger.info("Updating product with data: %s", product_data)
    product.deserialize(product_data)
    product.id = product_id
    product.update()
    app.logger.info("Updated product: %s", product.serialize())
    return product.serialize(), status.HTTP_200_OK
